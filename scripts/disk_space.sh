...
if [ "$CURRENT" -gt "$AM_percentage" ] ; then
	...
	AM_MY_IP=$(curl ifconfig.co)

	$AM_LOCATION/auth/generate_token.sh
	SECURE_TOKEN=$(cat $AM_LOCATION/auth/secure.token)

	AM_WEBHOOK_URL="${curl ifconfig.co}:1337/hooks/f?t=${SECURE_TOKEN}&f="
	...
	#send mail
	php mail/sendmail.php $(cat $AM_LOCATION/config.json | jq -r '.email') "Disk Full" "${AM_SMS_CONTENT_HTML}" "${AM_SMS_CONTENT}"
